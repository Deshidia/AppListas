...
import com.lagradost.quicknovel.databinding.MainPageSectionDividerBinding
...
class MainAdapter(
    ...
    override fun customContentViewType(item: SearchResponse): Int =
        if (item.isSectionDivider) VIEW_TYPE_DIVIDER else VIEW_TYPE_PRODUCT
    ...
    override fun onCreateCustomContent(parent: ViewGroup, viewType: Int): ViewHolderState<Any> {
        if (viewType == VIEW_TYPE_DIVIDER) {
            return ViewHolderState(
                MainPageSectionDividerBinding.inflate(
    ...
    override fun onBindContent(
        ...
        if (item.isSectionDivider) {
            (holder.view as MainPageSectionDividerBinding).sectionDividerText.text = item.name
            return
        }
        ...
        (holder.view as SearchResultGridBinding).apply {
            ...
            imagePrice.text = item.priceLabel
            imagePrice.isVisible = !item.price.isNullOrBlank()

            imageDiscount.text = item.discountLabel
            imageDiscount.isVisible = item.discountLabel != null
        }
    }
