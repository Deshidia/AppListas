...
class MainPageFragment : BaseFragment<FragmentMainpageBinding>(
    ...
    override fun onBindingCreated(binding: FragmentMainpageBinding, savedInstanceState: Bundle?) {
        ...
        binding.mainpageList.apply {
            ...
            // Section-divider rows (eg. small-category titles) span the full row width
            // instead of taking up a single grid cell.
            setSpanSizeLookup { position ->
                if (mainPageAdapter.getItemOrNull(position)?.isSectionDivider == true) spanCount else 1
            }
